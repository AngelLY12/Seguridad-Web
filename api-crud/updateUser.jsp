<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ page import="modelo.Celular" %> 
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Actualizar Celulares</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100">
    <header class="bg-blue-600 text-white py-4">
        <div class="container mx-auto text-center">
            <h1 class="text-3xl font-bold">Formulario de Actualización</h1>
            <a href="/api-crud/SelectController?" class="hover:underline underline-offset-8">Ver celulares</a>
        </div>
    </header>
    <main class="container mx-auto p-6">
        <form action="${pageContext.request.contextPath}/UpdateController" method="POST" class="bg-white shadow-md rounded-lg p-6">
            <div class="space-y-6">
                <h2 class="text-2xl font-semibold text-gray-800 mb-4">Agrega tu celular</h2>
                <% String errorMessage = (String) request.getAttribute("errorMessage"); %>
                <% if (errorMessage != null) { %>
                    <div style="color: red; border: 1px solid red; padding: 10px; margin-bottom: 10px;">
                        <strong>Error:</strong> <%= errorMessage %>
                    </div>
                <% } %>
                <div>
                    <label for="rfc" class="block text-sm font-medium text-gray-700">RFC</label>
                    <input
                    value="<%= request.getParameter("rfc") %>"
                    type="text"
                        id="rfc"
                        name="rfc"
                        placeholder="Escribe el IMEI de tu celular"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        readonly
                    />
                    <small class="w-full h-10" id="rfc-message"></small>

                </div>
        
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nombre</label>
                    <input
                    value="<%= request.getParameter("name") %>"
                    type="text"
                        id="name"
                        name="name"
                        placeholder="Escribe el nombre de tu celular"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        maxlength="20"

                        
                    />
                </div>
        
                <div>
                    <label for="lastname" class="block text-sm font-medium text-gray-700">Año de Lanzamiento</label>
                    <input
                    value="<%= request.getParameter("lastName") %>"
                    type="text"
                        id="lastName"
                        name="lastName"
                        placeholder="Escribe el año de lanzamiento"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        maxlength="20"
                    />

                </div>
        
                <div>
                    <label for="profile" class="block text-sm font-medium text-gray-700">Perfil</label>
                    <select 
                        id="profile" 
                        name="profile" 
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    >
                        <option value="user" <%= "user".equals(request.getParameter("profile")) ? "selected" : "" %>>USER</option>
                        <option value="moderator" <%= "moderator".equals(request.getParameter("profile")) ? "selected" : "" %>>MODERATOR</option>
                    </select>
                </div>
                
                
                <div class="text-right">
                    <button
                        type="submit"
                        class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                    >
                        Actualizar
                    </button>
                    <button id="cancelButton" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
                    >
                        Cancelar
                    </button>
                </div>
            </div>
        </form>
    </main>
    <script src="validation.js" defer></script>

    <script>
        const cancelButton = document.getElementById("cancelButton");
    
        cancelButton.addEventListener("click", function(event) {
            event.preventDefault(); 
            history.back();
        });
    </script>

</body>
</html>